...
plan 36 (deploy flow → `RUNBOOK.md`), plan 31 (app-layer security),
plan 33 (database backups/restore).

Scope: everything between "ordered the VPS" and "Coolify is ready for its
first deploy". App-layer security (headers, rate limits, cookies) is plan 31;
deploy/rollback procedure is plan 36; pg_dump backups and the restore drill
are plan 33. This runbook is the layer under all three: the OS, SSH, the
...
| Automated backup | Yes (~$1.80/mo) — full-VPS daily snapshot, distinct from plan 33's pg_dump |
...
itself — plan 31. Fail2Ban here covers SSH only.
...
Deploy wiring, notifications, and rollback rehearsal are plan 36 — stop
...
- Disk watch: `df -h /` weekly (or via plan 32 monitoring when it lands).
...
- Once proxied, the app sees Cloudflare's IPs — plan 31's rate limiting
  ...
  in the plan 31 PR if Cloudflare is in front by then.
...
- **App-layer security** (headers, HSTS, rate limits, cookie flags) —
  plan 31, in code, testable.
- **Deploy/rollback/notifications** — plan 36, `RUNBOOK.md`.
- **Database backups + restore drill** — plan 33. The OVHcloud automated
...
- Lost the VPS entirely: re-run this runbook on a fresh VPS (≈30 min),
  restore DB per plan 33's drill, repoint DNS. The VPS is cattle; the
  database is the pet — which is why plan 33's off-VPS backups are the one
