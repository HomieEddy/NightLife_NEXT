...
async function liveGET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json(await listEventGuests(db, id));
}
...
async function livePOST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = zEventGuestInput.safeParse({ ...body, eventId: id });
  if (!parsed.success) return NextResponse.json({ error: parsed.error.message }, { status: 400 });

  const { venueId } = sessionToDbContext(auth.session);
  const db = getDb({ venueId });
  const guest = await addEventGuest(db, parsed.data);
