...
async function livePATCH(
  ...
  const { setEventGuestStatus } = await import("@/features/hospitality/events-core");
  const { zEventGuestStatus } = await import("@/features/hospitality/events-schemas");

  const auth = await requireApiArea("manager");
  if ("error" in auth) return NextResponse.json({ error: auth.error }, { status: auth.status });

  const body = await request.json();
  const parsed = zEventGuestStatus.safeParse(body.status);
  if (!parsed.success) return NextResponse.json({ error: parsed.error.message }, { status: 400 });

  const { guestId } = await params;
  const { venueId } = sessionToDbContext(auth.session);
  const db = getDb({ venueId });
  const guest = await setEventGuestStatus(db, guestId, parsed.data);
