...
async function liveGET(request: NextRequest) {
  ...
  const { listItems } = await import("@/features/menu/core");
...
async function livePOST(request: NextRequest) {
  const { requireApiArea, sessionToDbContext } = await import("@/features/platform/auth-helpers");
  const { getDb } = await import("@/features/shared/db");
  const { createItem } = await import("@/features/menu/core");
  const { zItemInput } = await import("@/features/menu/schemas");

  const auth = await requireApiArea("manager");
  if ("error" in auth) return NextResponse.json({ error: auth.error }, { status: auth.status });

  const parsed = zItemInput.safeParse(await request.json());
  if (!parsed.success) return NextResponse.json({ error: parsed.error.message }, { status: 400 });

  const { venueId } = sessionToDbContext(auth.session);
  const db = getDb({ venueId });
  const item = await createItem(db, venueId, parsed.data);
